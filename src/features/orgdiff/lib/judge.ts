...
Общие нормы (вести переписку, взаимодействовать «по всему кругу вопросов», выполнять прочие поручения, участвовать в разработке документов) НЕ покрывают конкретную функцию, если в них не названо то же действие над тем же объектом.
...
Общие нормы (вести переписку, взаимодействовать «по всему кругу вопросов», выполнять прочие поручения, участвовать в разработке документов) НЕ покрывают конкретную функцию, если в них не названо то же действие над тем же объектом. Такое покрытие — не больше чем partially.
...
- duplication — одно и то же действие над тем же объектом закреплено за разными подразделениями, даже если формулировки немного различаются (например, оба «анализируют результаты непрерывного аудита» или оба «организуют контроль устранения недостатков»);
- overlap — действия или объекты различаются, но зоны ответственности частично пересекаются (например, «дата-аналитика» и «непрерывный мониторинг»);
...
export const COI_RULES = [
  ...
  'R10. Одно подразделение или один пункт совмещает закупку (заказ) или приёмку работ/услуг с контролем (в т.ч. внутренним финансовым контролем) тех же закупок — нарушение разделения обязанностей.'
];
...
// ---------- 0. Сопоставимы ли комплекты «до» и «после» ----------
...
const COMPARABILITY_SCHEMA = S.obj({
  comparable: { type: 'boolean' },
  reason: S.str('1–2 предложения для пользователя: что загружено в «до» и «после» и почему их можно или нельзя сравнивать')
});

export async function judgeComparability(before: string, after: string): Promise<Comparability> {
  const prompt = `Пользователь загрузил два комплекта документов, чтобы сравнить организационную структуру и функции ДО и ПОСЛЕ реорганизации. Реши, относятся ли комплекты к одному объекту (одна организация / один блок или подразделение и его преемники, редакции одних и тех же положений, приказ о реорганизации и т.п.) и имеет ли смысл их сравнивать. comparable=false ТОЛЬКО если комплекты явно о разных, не связанных подразделениях или сферах деятельности, либо на одной из сторон посторонний документ (не об оргструктуре и функциях). Если стороны относятся к одной организации, блоку или сфере — даже когда одна сторона представлена лишь приказом, частью документов или другим типом документа — comparable=true. При сомнении — comparable=true: лишний анализ лучше ложного отказа.
...
ПОСЛЕ:
${after}`;
  return llmJson<Comparability>('judge_comparability', COMPARABILITY_SCHEMA, prompt);
}
...
export async function writeConclusion(digest: string): Promise<ConclusionDraft> {
  const prompt = `Составь итоговое аналитическое заключение по результатам сравнения оргструктуры и функций ДО и ПОСЛЕ реорганизации. Пиши понятно для сотрудника, проводящего анализ: разделы «Изменения структуры», «Потеря и сужение функций», «Дублирование и пересечения», «Конфликт интересов» (пропусти раздел, если по нему нет выводов). Опирайся ТОЛЬКО на выводы ниже, в findingIds указывай id выводов, на которых основан текст. Сначала излагай ИЗМЕНЕНИЯ новой редакции (выводы с пометкой «новое»); выводы с пометкой «было ранее» — отдельным последним разделом «Замечания к структуре в целом». Резюме начинай с изменений. Итоговые рекомендации не должны противоречить «Рекомендациям вывода»: опирайся на них. Формулировки — гипотезы, требующие проверки. Рекомендации — конкретные (кому передать функцию, как разделить обязанности).
