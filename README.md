...
5. Контрольный комплект с заранее известными изменениями: «до» = `data/before_polozhenie_red8.docx`, «после» = `data/control/after_red9_control.txt`. **Ожидаемо:** ДККМ преобразован в ДМК (функции перешли в ДМК); потеря функции «обучение работников БВА» (до п. 5.5.9); дублирование «оценки качества» у ДНМ (п. 5.4.11) и ДМК (п. 5.5.2). В нём удалена функция (обучение работников ДККМ), одна функция продублирована в ДНМ, а ДККМ переименован в ДМК.
