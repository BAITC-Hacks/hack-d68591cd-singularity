...
`bun run eval` сверяет результат с экспертным эталоном `data/control/GOLD.md`: более 50 размеченных изменений, цитаты проверены по тексту. Затем прогоняет контрольный комплект. Текущий результат: **16/16 проверок**, 100% цитат найдены в текстах пунктов.
