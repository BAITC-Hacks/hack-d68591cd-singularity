...
- Анализ новых документов занимает 2–3 минуты (модель рассуждает); ответы кэшируются в `data/cache/`.
...
**Модели и данные.** OpenAI `gpt-6-astra` и `text-embedding-3-large` через API. Тестовые документы предоставлены организатором. Эталон `data/control/GOLD.md` размечен ИИ-агентами (три независимые разметки со сведением) с автоматической проверкой каждой цитаты по тексту документа.
